...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'instance', 'securesphere.db')
...
class User(db.Model):
    ...
    def unassign_client(self, client_id):
        """Unassign a client from this lead user (removes from both old and new assignment systems)"""
        if self.role != 'lead':
            return False
        client = User.query.filter_by(id=client_id, role='client').first()
        removed = False
        # Remove from new many-to-many
        if client and client in self.assigned_clients:
            self.assigned_clients.remove(client)
            removed = True
        # Remove from old assignment if matches
        if hasattr(self, 'assigned_client_id') and self.assigned_client_id == client_id:
...
def calculate_overall_maturity_score(product_id, user_id):
    ...
    1. For each section/dimension, calculate average score: Sum of scores for all questions in section / Number of questions in section
    2. Overall Maturity Score = Sum of average scores of all sections / Number of sections
    
    The mapping is fixed: A=Level 1, B=Level 2, C=Level 3, D=Level 4, E=Level 5
    """
    ...
    # Group responses by section/dimension
    section_questions = {}
    ...
    for response in responses:
        ...
        # Calculate score for this question (1-5 scale based on CSV mapping A=1, B=2, C=3, D=4, E=5)
    ...
    # Calculate average section scores and overall score
    section_averages = []
    section_data = {}
    
    for section, questions in section_questions.items():
        # Average Section Score = Sum of scores for all questions in section / Number of questions in section
        total_score = sum(q['score'] for q in questions)
        num_questions = len(questions)
        average_section_score = total_score / num_questions if num_questions > 0 else 0
        
        section_averages.append(average_section_score)
        section_data[section] = {
            'average_score': round(average_section_score, 2),
            'maturity_score': round(average_section_score, 2),
            'questions_count': num_questions,
            'total_score': total_score,
            'questions': questions
        }
    
    # Overall Maturity Score = Sum of average scores of all sections / Number of sections
    if len(section_averages) > 0:
        overall_score = sum(section_averages) / len(section_averages)
    else:
        overall_score = 0
...
def calculate_subdimension_scores(product_id, user_id):
    """
    Calculate scores by main dimensions (5 categories) using CSV structure
    Returns a dictionary with main dimension names as keys and their calculated scores
    """
    # Get all responses for this product and user
    responses = QuestionnaireResponse.query.filter_by(
        product_id=product_id, user_id=user_id
    ...
    # Read CSV to map questions to their main dimensions
    csv_map = {}
    dimension_questions = {}
    ...
    try:
        ...
        with open(csv_path, encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # Track current main dimension
                if dimension:
                    current_dimension = dimension
                    
                # Map question to main dimension
                if question and current_dimension:
                    csv_map[question] = current_dimension
                    
                    # Initialize main dimension tracking
                    if current_dimension not in dimension_questions:
                        dimension_questions[current_dimension] = []
                        
    except FileNotFoundError:
        print("CSV file not found")
        return {}
    
    # Group responses by main dimension
    for response in responses:
        dimension = csv_map.get(response.question)
        if dimension and dimension in dimension_questions:
    ...
    # Calculate main dimension scores using same logic: sum of scores / number of questions
    dimension_scores = {}
    
    for dimension, questions in dimension_questions.items():
        if questions:  # Only process dimensions that have questions
            total_score = sum(q['score'] for q in questions)
            num_questions = len(questions)
...
def calculate_actual_subdimension_scores(product_id, user_id):
    """
    Calculate scores by actual sub-dimensions using CSV structure with fixed mapping A=1, B=2, C=3, D=4, E=5
    For each sub-dimension, determine current maturity level by taking the most representative level 
    from all its related questions (average rounded to nearest whole level)
    ...
    for response in responses:
        ...
        if subdimension and subdimension in subdimension_questions:
            # Calculate score for this question using fixed mapping A=1, B=2, C=3, D=4, E=5
            question_score = calculate_question_score_from_csv(response.question, response.answer)
    ...
    # Calculate sub-dimension scores: sum of scores / number of questions
    subdimension_scores = {}
    ...
    for subdimension, questions in subdimension_questions.items():
        if questions:  # Only process sub-dimensions that have questions
            total_score = sum(q['score'] for q in questions)
            num_questions = len(questions)
            # Most representative level = average rounded to nearest whole level
            average_score = total_score / num_questions if num_questions > 0 else 0
...
def load_dsomm_roadmap():
    """Load DSOMM roadmap data from Excel file"""
    try:
        excel_path = os.path.join(os.path.dirname(__file__), 'DSOMM - Roadmap 1.xlsx')
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            roadmap_data[sheet_name] = df.to_dict('records')
        
        return roadmap_data
    except Exception as e:
        print(f"Error loading DSOMM roadmap: {e}")
        return {}

def get_maturity_level_from_score(score):
    """
    Map score to maturity level (1-5) using flooring logic:
    4.5+ → Level 5
    4.0+ → Level 4
    3.0+ → Level 3
    2.0+ → Level 2
...
def get_level_transition_recommendation(subdimension, current_level, target_level, dsomm_data):
    """
    Get recommendation from predefined library based on sub-dimension and level transition
    Format: 'L{current}→L{target}: {specific recommendation}'
    """
    
    # Enhanced level transition recommendations library
    level_transition_recommendations = {
        # Build and Deployment Sub-dimensions
        'Build': {
            '1→2': 'L1→L2: Document basic build procedures and establish consistent practices',
            '2→3': 'L2→L3: Implement automated build process with dependency management',
            '3→4': 'L3→L4: Add build security scanning and artifact verification',
            '4→5': 'L4→L5: Optimize build pipeline with advanced security controls and monitoring'
        },
        'Deployment': {
            '1→2': 'L1→L2: Create deployment documentation and standard procedures',
            '2→3': 'L2→L3: Implement consistent deployment process with automated checks',
            '3→4': 'L3→L4: Add blue/green deployment strategy with rollback capabilities',
            '4→5': 'L4→L5: Implement zero-downtime deployment with comprehensive monitoring'
        },
        'Patch Management': {
            '1→2': 'L1→L2: Establish basic patch tracking and vulnerability management',
            '2→3': 'L2→L3: Implement regular patch cycles with testing procedures',
            '3→4': 'L3→L4: Automate patch deployment with dependency analysis',
            '4→5': 'L4→L5: Implement predictive patch management with risk assessment'
        },
        
        # Culture and Organization Sub-dimensions
        'Design': {
            '1→2': 'L1→L2: Introduce basic security design principles and guidelines',
            '2→3': 'L2→L3: Implement threat modeling in design phase',
            '3→4': 'L3→L4: Integrate security architecture reviews and risk assessment',
            '4→5': 'L4→L5: Implement continuous security design optimization and feedback loops'
        },
        'Education and Guidance': {
            '1→2': 'L1→L2: Provide basic security awareness training for all teams',
            '2→3': 'L2→L3: Implement role-specific security training programs',
            '3→4': 'L3→L4: Establish security champions program with advanced training',
            '4→5': 'L4→L5: Implement continuous learning with security skill certification'
        },
        'Process': {
            '1→2': 'L1→L2: Document basic security processes and workflows',
            '2→3': 'L2→L3: Integrate security into development lifecycle (SDLC)',
            '3→4': 'L3→L4: Implement automated security process compliance checking',
            '4→5': 'L4→L5: Optimize processes with continuous improvement and metrics'
        },
        
        # Implementation Sub-dimensions
        'Application Hardening': {
            '1→2': 'L1→L2: Implement basic application security controls and configuration',
            '2→3': 'L2→L3: Apply OWASP Top 10 security controls systematically',
            '3→4': 'L3→L4: Implement advanced security controls with runtime protection',
            '4→5': 'L4→L5: Achieve comprehensive security hardening with adaptive controls'
        },
        'Development and Source Control': {
            '1→2': 'L1→L2: Establish secure coding standards and code review processes',
            '2→3': 'L2→L3: Implement branch protection and commit signing',
            '3→4': 'L3→L4: Add automated security checks in source control workflow',
            '4→5': 'L4→L5: Implement comprehensive DevSecOps with full traceability'
        },
        'Infrastructure Hardening': {
            '1→2': 'L1→L2: Apply basic infrastructure security configurations',
            '2→3': 'L2→L3: Implement infrastructure as code with security baselines',
            '3→4': 'L3→L4: Add automated compliance checking and drift detection',
            '4→5': 'L4→L5: Achieve self-healing infrastructure with adaptive security'
        },
        
        # Test and Verification Sub-dimensions
        'Application Tests': {
            '1→2': 'L1→L2: Implement basic security testing in application lifecycle',
            '2→3': 'L2→L3: Add automated security tests to CI/CD pipeline',
            '3→4': 'L3→L4: Implement comprehensive security test coverage',
            '4→5': 'L4→L5: Achieve continuous security testing with AI-driven optimization'
        },
        'Consolidation': {
            '1→2': 'L1→L2: Establish basic security testing consolidation practices',
            '2→3': 'L2→L3: Implement centralized security testing dashboard',
            '3→4': 'L3→L4: Add automated test result correlation and analysis',
            '4→5': 'L4→L5: Achieve comprehensive security testing orchestration'
        },
        'Dynamic Depth for Applications': {
            '1→2': 'L1→L2: Implement basic dynamic application security testing (DAST)',
            '2→3': 'L2→L3: Add DAST to CI/CD with automated scheduling',
            '3→4': 'L3→L4: Implement interactive application security testing (IAST)',
            '4→5': 'L4→L5: Achieve continuous dynamic testing with runtime insights'
        },
        'Dynamic Depth for Infrastructure': {
            '1→2': 'L1→L2: Implement basic infrastructure vulnerability scanning',
            '2→3': 'L2→L3: Add automated infrastructure security testing',
            '3→4': 'L3→L4: Implement runtime infrastructure security monitoring',
            '4→5': 'L4→L5: Achieve continuous infrastructure security validation'
        },
        'Static Depth for Applications': {
            '1→2': 'L1→L2: Implement basic static application security testing (SAST)',
            '2→3': 'L2→L3: Integrate SAST tools into development workflow',
            '3→4': 'L3→L4: Add multiple SAST tools with custom rule sets',
            '4→5': 'L4→L5: Achieve optimal SAST with continuous rule optimization'
        },
        'Static Depth for Infrastructure': {
            '1→2': 'L1→L2: Implement basic infrastructure as code security scanning',
            '2→3': 'L2→L3: Add policy-based infrastructure security validation',
            '3→4': 'L3→L4: Implement comprehensive infrastructure security analysis',
            '4→5': 'L4→L5: Achieve optimal infrastructure security scanning'
        },
        'Test-Intensity': {
            '1→2': 'L1→L2: Establish basic security testing frequency and coverage',
            '2→3': 'L2→L3: Implement regular intensive security testing cycles',
            '3→4': 'L3→L4: Add risk-based intensive testing with automation',
            '4→5': 'L4→L5: Achieve optimal testing intensity with adaptive scheduling'
        },
        
        # Information Gathering Sub-dimensions
        'Logging': {
            '1→2': 'L1→L2: Implement basic security logging and log collection',
            '2→3': 'L2→L3: Add centralized logging with security event correlation',
            '3→4': 'L3→L4: Implement advanced log analysis with threat detection',
            '4→5': 'L4→L5: Achieve optimal logging with AI-driven security insights'
        },
        'Monitoring': {
            '1→2': 'L1→L2: Establish basic security monitoring and alerting',
            '2→3': 'L2→L3: Implement comprehensive security monitoring dashboard',
            '3→4': 'L3→L4: Add behavioral analysis and anomaly detection',
            '4→5': 'L4→L5: Achieve optimal monitoring with predictive security analytics'
        },
        'Test KPI': {
            '1→2': 'L1→L2: Define basic security testing metrics and KPIs',
            '2→3': 'L2→L3: Implement automated KPI collection and reporting',
            '3→4': 'L3→L4: Add advanced analytics and trend analysis for KPIs',
            '4→5': 'L4→L5: Achieve optimal KPI insights with predictive analytics'
        }
    }
    
    # Create transition key
    transition_key = f"{current_level}→{target_level}"
    
    # Try to find specific recommendation
    if subdimension in level_transition_recommendations:
        subdim_recommendations = level_transition_recommendations[subdimension]
        if transition_key in subdim_recommendations:
            return subdim_recommendations[transition_key]
    
    # Try partial matching for subdimensions not exactly matching
    for key, recommendations in level_transition_recommendations.items():
        if key.lower() in subdimension.lower() or subdimension.lower() in key.lower():
            if transition_key in recommendations:
                return recommendations[transition_key]
    
    # Fallback to generic recommendation
    generic_recommendations = {
        '1→2': f'L1→L2: Establish basic {subdimension.lower()} security practices and documentation',
        '2→3': f'L2→L3: Implement consistent {subdimension.lower()} process with automation',
        '3→4': f'L3→L4: Add advanced {subdimension.lower()} controls with monitoring',
        '4→5': f'L4→L5: Optimize {subdimension.lower()} with continuous improvement',
        '5→5': f'L5: Maintain optimized {subdimension.lower()} practices with ongoing refinement'
    }
    
    return generic_recommendations.get(transition_key, f'Continue improving {subdimension.lower()} security practices')

def get_improvement_phase(current_level):
    """
    Assign improvement phase based on current maturity level:
    - Phase 0-3 months: Level 1 or Level 2 (highest priority fixes)  
    - Phase 3-6 months: Level 3 (medium-term improvements)
    - Phase 6-12 months: Level 4 (optimizations to reach Level 5)
    """
    if current_level <= 2:
        return "0-3 months (Highest Priority)"
    elif current_level == 3:
        return "3-6 months (Medium Priority)" 
    elif current_level == 4:
        return "6-12 months (Optimization)"
    else:
        return "Maintenance (Level 5 Achieved)"

def get_phase_priority(current_level):
    ...
    if current_level <= 2:
        ...
        return 3  # Lower priority (optimization)
    else:
        return 4  # Maintenance

def generate_subdimension_roadmap(product_id, user_id):
    """
    Generate sub-dimension wise roadmap recommendations using the specified logic:
    - Calculate current maturity level for each sub-dimension
    - Determine target level and recommendations
    - Group by phases based on current levels
    """
    # Get sub-dimension scores
    subdimension_scores = calculate_actual_subdimension_scores(product_id, user_id)
    
    if not subdimension_scores:
        return []
    
    # Load DSOMM roadmap data
    dsomm_data = load_dsomm_roadmap()
    
    # Calculate overall maturity level
    overall_score, _ = calculate_overall_maturity_score(product_id, user_id)
    current_overall_level = get_maturity_level_from_score(overall_score) if overall_score and overall_score > 0 else 1
    
    roadmap = []
    
    for subdim_name, data in subdimension_scores.items():
        # Determine current maturity level from average score
        current_level = get_maturity_level_from_score(data['average_score'])
        
        # Target level is normally one level higher unless already at Level 5
        target_level = min(5, current_level + 1) if current_level < 5 else 5
        ...
        if current_level < current_overall_level:
            ...
            phase_priority = 1
        elif current_level == current_overall_level:
        ...
        roadmap_item = {
            'subdimension': subdim_name,
            'current_score': round(data['average_score'], 2),
    ...
    # Sort by phase priority (lowest level gaps first)
    roadmap.sort(key=lambda x: (x['phase_priority'], x['current_level']))
    
    return roadmap

def generate_overall_roadmap(product_id, user_id):
    """
    Generate roadmap phases using the specified logic:
    
    1. Calculate Overall Maturity Score
    2. Determine Current Maturity Level using flooring logic
    3. Calculate Subdimension Scores  
    4. Generate Roadmap Phases:
       - Phase 1 (0-3 months): Focus on bringing all sub-dimensions up to the current maturity level
       - Phase 2 (3-6 months): Target the next maturity level (current + 1)
       - Phase 3 (6-12 months): Target Level 5 (Optimized)
    5. Get Recommendations for each sub-dimension and target level
    6. Combine and Present
    """
    
    # Step 1: Calculate Overall Maturity Score
    overall_score, section_data = calculate_overall_maturity_score(product_id, user_id)
    
    # Step 2: Determine Current Maturity Level using flooring logic
    current_level = get_maturity_level_from_score(overall_score) if overall_score and overall_score > 0 else 1
    
    # Step 3: Calculate Subdimension Scores
    subdimension_scores = calculate_actual_subdimension_scores(product_id, user_id)
    ...
    # Step 4: Generate Roadmap Phases
    roadmap = []
    
    # Phase 1 (0-3 months): Focus on bringing all sub-dimensions up to the current maturity level
    subdims_below_current = []
    for subdim_name, data in subdimension_scores.items():
        subdim_level = get_maturity_level_from_score(data['average_score'])
        if subdim_level < current_level:
            subdims_below_current.append(subdim_name)
    
    if subdims_below_current:
        focus_areas_phase1 = []
        recommendations_phase1 = []
        
        for subdim in subdims_below_current:
            focus_area = get_focus_area_from_subdimension(subdim)
            if focus_area not in focus_areas_phase1:
                focus_areas_phase1.append(focus_area)
        ...
        roadmap.append({
            ...
            'goal': f'Bring all sub-dimensions up to current maturity level ({current_level})',
            'purpose': 'Address sub-dimensions below current maturity level',
            'focus_areas': focus_areas_phase1,
            'recommendations': recommendations_phase1,
            'subdimensions_to_improve': subdims_below_current,
            'is_combined': False,
            'priority': 'Highest',
            'description': f'Critical improvements to achieve Level {current_level} across all areas'
        })
    ...
    if next_level > current_level:
        ...
        for subdim_name, data in subdimension_scores.items():
            focus_area = get_focus_area_from_subdimension(subdim_name)
            if focus_area not in focus_areas_phase2:
                focus_areas_phase2.append(focus_area)
        ...
        roadmap.append({
            ...
            'goal': f'Advance all areas to Level {next_level}',
            'purpose': f'Target the next maturity level (Level {next_level})',
            'focus_areas': focus_areas_phase2,
            'recommendations': recommendations_phase2,
            'subdimensions_to_improve': list(subdimension_scores.keys()),
            'is_combined': False,
            'priority': 'Medium',
            'description': f'Systematic advancement to Level {next_level} maturity'
        })
    ...
    if current_level < 5:
        ...
        for subdim_name, data in subdimension_scores.items():
            focus_area = get_focus_area_from_subdimension(subdim_name)
            if focus_area not in focus_areas_phase3:
                focus_areas_phase3.append(focus_area)
        ...
        roadmap.append({
            'phase': 'Phase 3 (6-12 months)',
            'target_level': 5,
            'goal': 'Achieve Level 5 (Optimized) across all areas',
            'purpose': 'Target Level 5 (Optimized) for continuous improvement',
            'focus_areas': focus_areas_phase3,
            'recommendations': recommendations_phase3,
            'subdimensions_to_improve': list(subdimension_scores.keys()),
            'is_combined': False,
            'priority': 'Optimization',
            'description': 'Advanced optimization to achieve peak security maturity'
        })
    
    # Add maintenance phase if already at Level 5
    if current_level >= 5:
        roadmap.append({
            'phase': 'Maintenance (Ongoing)',
            'target_level': 5,
            'goal': 'Maintain Level 5 excellence across all security dimensions',
            'purpose': 'Continuous monitoring and refinement of optimized practices',
            'focus_areas': get_all_focus_areas(),
            'recommendations': [
                'Monitor security metrics and KPIs for any degradation',
                'Regularly review and update security practices',
                'Conduct periodic DSOMM assessments for continuous improvement',
                'Stay current with emerging security threats and technologies',
                'Share security best practices across the organization'
            ],
            'subdimensions_to_improve': [],
            'is_combined': False,
            'priority': 'Maintenance',
            'description': 'Ongoing maintenance of achieved security excellence'
        })
    
    return roadmap

def get_focus_area_from_subdimension(subdimension):
    """Map sub-dimension to main focus area"""
    focus_area_mapping = {
        'Build': 'Build and Deployment',
        'Deployment': 'Build and Deployment', 
        'Patch Management': 'Build and Deployment',
        'Design': 'Culture and Organization',
        'Education and Guidance': 'Culture and Organization',
        'Process': 'Culture and Organization',
        'Application Hardening': 'Implementation',
        'Application Tests': 'Test and Verification',
        'Consolidation': 'Test and Verification',
        'Development and Source Control': 'Implementation',
        'Dynamic Depth for Applications': 'Test and Verification',
        'Dynamic Depth for Infrastructure': 'Test and Verification',
        'Infrastructure Hardening': 'Implementation',
        'Logging': 'Information Gathering',
        'Monitoring': 'Information Gathering',
        'Static Depth for Applications': 'Test and Verification',
        'Static Depth for Infrastructure': 'Test and Verification',
        'Test KPI': 'Information Gathering',
        'Test-Intensity': 'Test and Verification'
    }
    
    # Try exact match first
    if subdimension in focus_area_mapping:
        return focus_area_mapping[subdimension]
    
    # Try partial match
    for key, value in focus_area_mapping.items():
        if key.lower() in subdimension.lower():
            return value
...
def get_combined_dsomm_recommendations(target_level, focus_areas, dsomm_data):
    """Get combined DSOMM recommendations for a specific level across focus areas"""
    
    # Level-specific combined recommendations based on your examples
    level_combined_recommendations = {
        1: {
            'Build and Deployment': [
                'Establish basic build documentation',
                'Implement manual deployment procedures',
                'Create initial patch tracking'
            ],
            'Culture and Organization': [
                'Introduce basic security awareness',
                'Establish initial design reviews',
                'Create basic security guidelines'
            ],
            'Implementation': [
                'Implement basic security controls',
                'Establish development standards',
                'Create basic hardening guidelines'
            ],
            'Test and Verification': [
                'Introduce manual security testing',
                'Establish basic test procedures',
                'Create initial verification processes'
            ],
            'Information Gathering': [
        ...
        3: {
            'Build and Deployment': [
                'Establish build process with SBOM implementation',
                'Implement code signing and artifact signing', 
                'Create patch management policy with automated dependency updates'
            ],
            'Culture and Organization': [
                'Integrate threat modeling into sprint planning',
                'Designate security champions and implement training',
                'Introduce security-focused unit and integration tests'
            ],
            'Implementation': [
                'Implement automated security scanning in CI/CD',
                'Implement secure development practices',
                'Establish basic infrastructure security controls'
            ],
            'Test and Verification': [
                'Enable custom alert routing and metric management',
                'Enforce static code analysis in development',
                'Set up basic dynamic application testing',
            ...
            'Information Gathering': [
                ...
                'Establish security testing metrics and KPIs'
            ]
        },
        4: {
            'Build and Deployment': [
                'Implement secure deployment process with blue/green strategy',
                'Formalize change management and asset classification',
                'Adopt OWASP ASVS framework for application hardening'
            ],
            'Culture and Organization': [
                'Implement infrastructure as code with security scanning',
                'Enhance event correlation and logging policies',
                'Integrate DAST tools into CI/CD pipeline'
            ],
            'Implementation': [
                'Implement comprehensive application security controls',
                'Advanced secure development workflows',
                'Implement advanced infrastructure security controls'
            ],
            'Test and Verification': [
                'Advanced application security testing',
                'Implement advanced DAST with comprehensive coverage',
                'Advanced SAST with multiple scanners',
                'Advanced security testing metrics and reporting',
                'Implement intensive security testing protocols'
            ],
            'Information Gathering': [
                'Advanced logging with correlation and analysis',
                'Implement advanced security monitoring and response',
                'Comprehensive infrastructure security scanning'
            ]
        },
        5: {
            'Build and Deployment': [
                'Implement comprehensive SAST tooling',
                'Use multiple scanners to reduce false negatives',
                'Track advanced metrics like patch deployment time and library age'
            ],
            'Culture and Organization': [
                'Audit and optimize all prior implementations',
                'Measure DSOMM progress with regular assessments',
                'Establish continuous security improvement program'
            ],
            'Implementation': [
                'Achieve full application security optimization',
                'Continuous improvement of secure development',
                'Achieve optimal infrastructure security posture'
            ],
            'Test and Verification': [
                'Implement continuous security testing optimization',
                'Optimize DAST with continuous improvement',
                'Optimize SAST with continuous improvement',
                'Achieve optimal security testing metrics and insights',
                'Implement optimal security testing intensity'
            ],
            'Information Gathering': [
                'Implement optimal logging with AI/ML analysis',
                'Achieve optimal security monitoring and response',
                'Optimal infrastructure security analysis'
            ]
        }
    }
    ...
    # Get recommendations for the target level and specified focus areas
    if target_level in level_combined_recommendations:
        level_recs = level_combined_recommendations[target_level]
        for focus_area in focus_areas:
            if focus_area in level_recs:
                combined_recommendations.extend(level_recs[focus_area])
    
    # If no specific recommendations found, try to get from DSOMM Excel data
    if not combined_recommendations:
        # Fallback to generic recommendations
        generic_recommendations = {
            1: "Establish basic security practices and awareness",
            2: "Implement fundamental security controls and processes", 
            3: "Integrate security into development workflows and CI/CD",
            4: "Automate security testing and implement advanced controls",
            5: "Optimize with continuous improvement and feedback loops"
        }
        combined_recommendations = [generic_recommendations.get(target_level, "Continue improving security practices")]
    
    return combined_recommendations

def get_dsomm_recommendation(subdimension, target_level, dsomm_data):
    """Get DSOMM recommendation for a specific sub-dimension and level"""
    
    # Enhanced specific recommendations based on DSOMM and your examples
    level_recommendations = {
        3: {
            'Build': 'Establish build process with SBOM implementation',
            'Deployment': 'Implement code signing and artifact signing',
            'Patch Management': 'Create patch management policy with automated dependency updates',
            'Design': 'Integrate threat modeling into sprint planning',
            'Education and Guidance': 'Designate security champions and implement training',
            'Process': 'Introduce security-focused unit and integration tests',
            'Application Hardening': 'Implement automated security scanning in CI/CD',
            'Application Tests': 'Enable custom alert routing and metric management',
            'Consolidation': 'Enforce static code analysis in development',
            'Development and Source Control': 'Implement secure development practices',
            'Dynamic Depth for Applications': 'Set up basic dynamic application testing',
            'Dynamic Depth for Infrastructure': 'Implement infrastructure security testing',
            'Infrastructure Hardening': 'Establish basic infrastructure security controls',
            'Logging': 'Implement centralized logging and monitoring',
            'Monitoring': 'Set up security monitoring and alerting',
            'Static Depth for Applications': 'Integrate SAST tools into CI/CD pipeline',
            'Static Depth for Infrastructure': 'Implement infrastructure as code scanning',
            'Test KPI': 'Establish security testing metrics and KPIs',
            'Test-Intensity': 'Implement comprehensive security testing strategy'
        },
        4: {
            'Build': 'Implement secure deployment process with blue/green strategy',
            'Deployment': 'Formalize change management and asset classification',
            'Patch Management': 'Adopt OWASP ASVS framework for application hardening',
            'Design': 'Implement infrastructure as code with security scanning',
            'Education and Guidance': 'Enhance event correlation and logging policies',
            'Process': 'Integrate DAST tools into CI/CD pipeline',
            'Application Hardening': 'Implement comprehensive application security controls',
            'Application Tests': 'Advanced application security testing',
            'Consolidation': 'Implement advanced security consolidation practices',
            'Development and Source Control': 'Advanced secure development workflows',
            'Dynamic Depth for Applications': 'Implement advanced DAST with comprehensive coverage',
            'Dynamic Depth for Infrastructure': 'Advanced infrastructure security testing',
            'Infrastructure Hardening': 'Implement advanced infrastructure security controls',
            'Logging': 'Advanced logging with correlation and analysis',
            'Monitoring': 'Implement advanced security monitoring and response',
            'Static Depth for Applications': 'Advanced SAST with multiple scanners',
            'Static Depth for Infrastructure': 'Comprehensive infrastructure security scanning',
            'Test KPI': 'Advanced security testing metrics and reporting',
            'Test-Intensity': 'Implement intensive security testing protocols'
        },
        5: {
            'Build': 'Implement comprehensive SAST tooling',
            'Deployment': 'Use multiple scanners to reduce false negatives',
            'Patch Management': 'Track advanced metrics like patch deployment time and library age',
            'Design': 'Audit and optimize all prior implementations',
            'Education and Guidance': 'Measure DSOMM progress with regular assessments',
            'Process': 'Establish continuous security improvement program',
            'Application Hardening': 'Achieve full application security optimization',
            'Application Tests': 'Implement continuous security testing optimization',
            'Consolidation': 'Optimize all security consolidation practices',
            'Development and Source Control': 'Continuous improvement of secure development',
            'Dynamic Depth for Applications': 'Optimize DAST with continuous improvement',
            'Dynamic Depth for Infrastructure': 'Full infrastructure security optimization',
            'Infrastructure Hardening': 'Achieve optimal infrastructure security posture',
            'Logging': 'Implement optimal logging with AI/ML analysis',
            'Monitoring': 'Achieve optimal security monitoring and response',
            'Static Depth for Applications': 'Optimize SAST with continuous improvement',
            'Static Depth for Infrastructure': 'Optimal infrastructure security analysis',
            'Test KPI': 'Achieve optimal security testing metrics and insights',
            'Test-Intensity': 'Implement optimal security testing intensity'
        }
    }
    
    # Helper function to check if a value is NaN or empty
    ...
    # Try to find specific recommendation in DSOMM data first
    try:
        for sheet_name, data in dsomm_data.items():
            for row in data:
                # Check if this row matches our subdimension (flexible matching)
                if any(subdimension.lower() in str(cell).lower() for cell in row.values() if cell and not pd.isna(cell)):
                    # Look for level-specific recommendations, starting from target_level and moving up if NaN
                    current_level = target_level
                    while current_level <= 5:
                        level_key = f'Level {current_level}'
                        for key, value in row.items():
                            if level_key in str(key):
                                if not is_nan_or_empty(value):
                                    return str(value).strip()
                        current_level += 1
                    
                    # If all levels from target_level to 5 are NaN, try lower levels
        ...
        print(f"Error reading DSOMM data: {e}")
    
    # Use enhanced default recommendations
    if target_level in level_recommendations and subdimension in level_recommendations[target_level]:
        return level_recommendations[target_level][subdimension]
    ...
    return generic_recommendations.get(target_level, "Continue improving security practices")
...
def calculate_question_score_from_csv(question, answer):
    ...
    Fixed mapping: A=Level 1, B=Level 2, C=Level 3, D=Level 4, E=Level 5
    """
    try:
        csv_path = os.path.join(os.path.dirname(__file__), 'devweb.csv')
        with open(csv_path, encoding='utf-8') as f:
    ...
    # Default scoring based on option letter if CSV parsing fails (A=1, B=2, C=3, D=4, E=5)
    if answer.startswith('A)'):
        return 1
    elif answer.startswith('B)'):
        return 2
...
@app.route('/')
def index():
    # Prevent infinite redirect loop by only redirecting if not already on dashboard or already requesting dashboard
    if 'user_id' in session and 'role' in session:
        # Only redirect if not already on dashboard or not requesting dashboard
...
def dashboard():
    ...
    if role == 'client':
        ...
        for product in products:
            ...
            rejected_responses = QuestionnaireResponse.query.filter_by(product_id=product.id, user_id=user_id, needs_client_response=True).all()
            ...
            for i, section in enumerate(SECTION_IDS):
                if section not in completed_sections:
                    next_section_idx = i
                    break
            latest_scores = ScoreHistory.query.filter_by(product_id=product.id, user_id=user_id).order_by(ScoreHistory.calculated_at.desc()).all()
            if latest_scores:
                dimension_scores = []
            ...
            product_unread_comments = LeadComment.query.filter_by(client_id=user_id, product_id=product.id, is_read=False).count()
            # Defensive: ensure all values are serializable and not Undefined
            def safe_val(val, typ, default):
                try:
                    if val is None:
                        return default
                    return typ(val)
            ...
            product_info = {
                ...
                'owner_id': safe_val(product.owner_id, int, 0),
                'status': safe_val(getattr(status_record, 'status', ''), str, ''),
                'completed_sections_count': safe_val(completed_sections_count, int, 0),
                'total_sections': safe_val(total_sections, int, 0),
        ...
        unread_chats_count = sum(getattr(chat, 'unread_messages_for_client', 0) for chat in active_chats)
        ...
        assigned_clients = current_lead.assigned_clients.filter_by(role='client', is_active=True).all()
        ...
        # Gather all products and responses for all assigned clients
        clients_data = {}
        total_products = 0
        in_progress_count = 0
        completed_count = 0
        for assigned_client in assigned_clients:
            all_client_products = Product.query.filter_by(owner_id=assigned_client.id).all()
            ...
            ).filter(User.id == assigned_client.id).all()
            clients_data[assigned_client.id] = {
            ...
            for product in all_client_products:
                ...
                is_complete = is_assessment_complete(product.id, assigned_client.id)
                product_responses = [resp for resp, user, prod in resps if prod.id == product.id]
                if is_complete and product_responses:
                    all_reviewed = all(resp.is_reviewed for resp in product_responses)
                    all_approved = all(resp.review_status in ['approved', 'approved_after_revision'] 
                                     for resp in product_responses if resp.is_reviewed)
                    ...
                    status = 'in_progress'
                clients_data[assigned_client.id]['products'][product.id] = {
                    'product': product,
        ...
        # Get unread client replies count for all assigned clients
        unread_client_replies = LeadComment.query.filter(
            LeadComment.client_id.in_([c.id for c in assigned_clients]),
            LeadComment.status == 'client_reply',
            LeadComment.is_read == False
        ).count()

        # Get active chats for lead's assigned clients
        active_chats_for_lead = QuestionChat.query.filter(
            QuestionChat.lead_id == user_id,
            QuestionChat.client_id.in_([c.id for c in assigned_clients]),
        ...
        # Organize all products and their data
        clients_data = {}
        products_status = {}  # Track status of each product
        ...
        if assigned_client:
            ...
            # Process all products for this client
            for product in all_client_products:
                # Get product status
                product_status = ProductStatus.query.filter_by(
                    product_id=product.id, 
                ...
                # Get responses for this product
                product_responses = [resp for resp, user, prod in resps if prod.id == product.id]
                
                # Determine product review status
                if is_complete and product_responses:
                    # Check if all responses are reviewed and approved
                    all_reviewed = all(resp.is_reviewed for resp in product_responses)
                    all_approved = all(resp.review_status in ['approved', 'approved_after_revision'] 
                                     for resp in product_responses if resp.is_reviewed)
                ...
                products_status[product.id] = status
                
                # Add to clients_data (include all products, not just complete ones)
                clients_data[assigned_client.id]['products'][product.id] = {
                    'product': product,
                    'responses': product_responses,
                    'status': status,
        ...
        in_progress_count = sum(1 for status in products_status.values() if status == 'in_progress')
        completed_count = sum(1 for status in products_status.values() if status == 'completed')
...
def product_results(product_id):
    try:
        ...
        product = Product.query.get_or_404(product_id)
        # Access control: allow client (owner), lead assigned to product, or superuser
        user_id = session.get('user_id')
        role = session.get('role')
        owner_id = getattr(product, 'owner_id', None)
        print(f"[DEBUG] product_id={product_id}, owner_id={owner_id}, user_id={user_id}, role={role}")
        # Check if user is owner (client)
        is_owner = user_id == owner_id
        # Check if user is lead assigned to this product
        is_lead = False
        if role == 'lead':
            try:
                owner = User.query.get(owner_id)
                is_lead = owner and any(l.id == user_id for l in owner.assigned_to_leads)
            except Exception:
                is_lead = False
        # Check if user is superuser/admin
        is_admin = role in ['admin', 'superuser']
        if not (is_owner or is_lead or is_admin):
            flash('Access denied: You do not have permission to view these results.', 'error')
            return redirect(url_for('dashboard'))
        
        # Always show the product owner's responses (assessment) for leads and admins
        target_user_id = owner_id
        
        # Check if assessment is complete before showing results
        is_complete = is_assessment_complete(product_id, target_user_id)
        if not is_complete:
        ...
        lead_comments_query = LeadComment.query.filter_by(product_id=product_id, client_id=owner_id).order_by(LeadComment.created_at.desc()).all()
        ...
        try:
            overall_score, section_data = calculate_overall_maturity_score(product_id, target_user_id)
        ...
        section_dimensions = {}
        try:
            subdimension_scores = calculate_actual_subdimension_scores(product_id, target_user_id)
        except Exception as e:
        ...
        try:
            heatmap_data = generate_heatmap_data(product_id, target_user_id)
        ...
        # Roadmap data is now handled by separate route
...
@app.route('/product/<int:product_id>/roadmap/<int:user_id>')
@login_required()
def product_roadmap(product_id, user_id):
    """Display product security roadmap"""
    try:
        # Check if user has access
        session_user_id = session.get('user_id')
        session_role = session.get('role')
        if not session_user_id:
            flash('Please log in to view roadmap.', 'warning')
            return redirect(url_for('login'))
        ...
        # Check if user is owner (client)
        is_owner = session_user_id == owner_id
        
        # Check if user is lead assigned to this product
        is_lead = False
        if session_role == 'lead':
            try:
                owner = User.query.get(owner_id)
                is_lead = owner and any(l.id == session_user_id for l in owner.assigned_to_leads)
            except Exception:
                is_lead = False
        
        # Check if user is superuser/admin
        is_admin = session_role in ['admin', 'superuser']
        
        if not (is_owner or is_lead or is_admin):
            flash('You do not have permission to view this roadmap.', 'error')
            return redirect(url_for('dashboard'))
        
        # Check if assessment is complete before showing roadmap
        is_complete = is_assessment_complete(product_id, user_id)
        if not is_complete:
        ...
        # Calculate dimension scores for compatibility
        dimension_scores = {}
        if section_data and isinstance(section_data, dict):
            for section_name, data in section_data.items():
                if isinstance(data, dict):
                    avg_score = float(data.get('average_score', 0)) if data.get('average_score') is not None else 0.0
                    question_count = int(data.get('questions_count', 0)) if data.get('questions_count') is not None else 0
                    total_score = float(data.get('total_score', 0)) if data.get('total_score') is not None else 0.0
...
@app.route('/admin/product/<int:product_id>/roadmap')
@login_required('superuser')
def admin_product_roadmap(product_id):
    """Admin view of product security roadmap"""
    try:
        # Get product information
        product = Product.query.get_or_404(product_id)
        owner = User.query.get(product.owner_id)
        ...
        if not owner:
            flash('Product owner not found. Cannot display roadmap.', 'error')
            return redirect(url_for('manage_clients'))
        
        # Check if assessment is complete before showing roadmap
        is_complete = is_assessment_complete(product_id, owner.id)
        if not is_complete:
            flash('Assessment must be completed before viewing roadmap. Please complete all questionnaire sections first.', 'warning')
            return redirect(url_for('manage_clients'))
        ...
        # Calculate sub-dimension scores
        subdimension_scores = calculate_actual_subdimension_scores(product_id, owner.id)
        ...
        if section_data and isinstance(section_data, dict):
            for section_name, data in section_data.items():
                if isinstance(data, dict) and 'responses' in data:
                    ...
                    if responses:
                        total_score = sum(resp.score for resp in responses if resp.score is not None)
                        question_count = len(responses)
                        avg_score = total_score / question_count if question_count > 0 else 0
                        dimension_scores[section_name] = {
                            'average_score': round(avg_score, 2),
                            'question_count': question_count,
                            'total_score': round(total_score, 2)
                        }
        ...
        # Generate roadmap data
        subdimension_roadmap = generate_subdimension_roadmap(product_id, owner.id)
        overall_roadmap = generate_overall_roadmap(product_id, owner.id)
...
def export_pdf(product_id):
    ...
    try:
        ...
        if not (is_owner or is_lead or is_admin):
            flash('Access denied: You do not have permission to export this report.', 'error')
            return redirect(url_for('dashboard'))
        
        # Get responses and scores for the product owner
        target_user_id = owner_id
        responses = QuestionnaireResponse.query.filter_by(
            product_id=product_id, user_id=target_user_id
        ).all()
        
        scores = ScoreHistory.query.filter_by(
            product_id=product_id, user_id=target_user_id
        ).all()
        ...
        if not responses:
            flash('No assessment data found for this product. Please complete the questionnaire first.', 'warning')
            return redirect(url_for('product_results', product_id=product_id))
        
        # Generate PDF filename
        product_name = getattr(product, 'name', None) or getattr(product, 'application_name', None) or "Product"
        safe_product_name = "".join(c for c in product_name if c.isalnum() or c in (' ', '-', '_')).rstrip()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"Security_Assessment_{safe_product_name}_{timestamp}.pdf"
        
        # Generate PDF with better error handling
        try:
            pdf_data = generate_product_pdf(product, responses, scores, user, filename)
        except Exception as pdf_error:
            print(f"PDF generation error: {pdf_error}")
            import traceback
            traceback.print_exc()
            flash(f'PDF generation failed: {str(pdf_error)}', 'error')
            return redirect(url_for('product_results', product_id=product_id))
...
def review_questionnaire(response_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            # Check if this question was previously revised (has active/inactive chat)
            existing_chat = QuestionChat.query.filter_by(response_id=response_id).first()
            if existing_chat:
                # This question went through revision process
                resp.review_status = 'approved_after_revision'
            else:
                # Direct approval without revision
...
def admin_product_results(product_id):
    ...
    # Get all responses for this product from the owner
    resps = QuestionnaireResponse.query.filter_by(product_id=product_id, user_id=owner.id).all()
    ...
    # Generate roadmap data
    subdimension_roadmap = []
    overall_roadmap = []
    try:
        subdimension_roadmap = generate_subdimension_roadmap(product_id, owner.id)
        overall_roadmap = generate_overall_roadmap(product_id, owner.id)
    except Exception as e:
        print(f'Error generating roadmap for admin: {e}')
    ...
    template_vars = {
        ...
        'subdimension_roadmap': subdimension_roadmap,  # Sub-dimension roadmap
        'overall_roadmap': overall_roadmap,  # Overall maturity roadmap
        'owner': owner,
        'is_admin_view': True
    }
...
def create_lead():
    ...
    flash(f'Lead user {username} created successfully and assigned to client {assigned_client.username}.')
    return redirect(url_for('manage_users'))
...
def delete_user(user_id):
    ...
    try:
        ...
        if user.role == 'client':
            # Delete products and responses
            products = Product.query.filter_by(client_id=user_id).all()
            for product in products:
                QuestionnaireResponse.query.filter_by(product_id=product.id).delete()
                db.session.delete(product)
        
        # Delete invitation tokens
        InvitationToken.query.filter_by(email=user.email).delete()
        
        # Remove from any lead assignments
        if user.role == 'client':
            # Remove client from all lead assignments
            leads = User.query.filter_by(role='lead').all()
            for lead in leads:
                lead.unassign_client(user_id)
        elif user.role == 'lead':
            # Remove all client assignments from this lead
            user.assigned_clients.clear()
...
@app.route('/admin/fix_assignments', methods=['POST'])
@login_required('superuser')
def fix_assignments():
    """Fix any inconsistencies in lead-client assignments"""
    try:
        # Get all leads with old-style assignments
        leads_with_old_assignments = User.query.filter(
            User.role == 'lead',
            User.assigned_client_id.isnot(None)
        ).all()
        
        for lead in leads_with_old_assignments:
            client = User.query.filter_by(id=lead.assigned_client_id, role='client').first()
            if client and client not in lead.assigned_clients.all():
                # Add to new many-to-many relationship
                lead.assigned_clients.append(client)
        
        db.session.commit()
        flash('Assignment consistency check completed successfully.', 'success')
    except Exception as e:
        db.session.rollback()
...
def admin_client_details(client_id):
    """View comprehensive details for a specific client and their products"""
    client = User.query.filter_by(id=client_id, role='client').first_or_404()
    
    return render_template('admin_client_details.html', client=client)
...
def view_question_chat(chat_id):
    ...
    chat = QuestionChat.query.options(
        db.joinedload(QuestionChat.response),
        db.joinedload(QuestionChat.client),
        db.joinedload(QuestionChat.lead),
        db.joinedload(QuestionChat.product)
...
def send_chat_message(chat_id):
    ...
    # If client sends a message, mark chat as completed (client has responded)
    if current_user_role == 'client' and chat.review_status in ['needs_revision', 'rejected']:
        chat.review_status = 'completed'
...
def approve_from_chat(chat_id):
    ...
    response.needs_client_response = False
    # Since this is approved from chat, it was definitely revised
    response.review_status = 'approved_after_revision'
...
def reselect_question(response_id):
    ...
    if request.method == 'POST':
        ...
        if existing_chat:
            ...
            old_answer = response.answer if hasattr(response, '_sa_instance_state') and response._sa_instance_state.expired else None
            reselection_message = ChatMessage(
                ...
                content=f'🔄 Client has reselected answer: "{new_answer}"' + 
                        ...
                        '\n\n✅ Scores and results have been recalculated.',
        ...
        # Refresh the response object to ensure we have the latest data
        db.session.refresh(response)
        if existing_chat:
            db.session.refresh(existing_chat)
...
@app.route('/reopen_question/<int:response_id>', methods=['GET', 'POST'])
@login_required('lead')
def reopen_question(response_id):
    """Re-open an approved question for review"""
    response = QuestionnaireResponse.query.get_or_404(response_id)
    
    # Check if the lead is authorized to review this client's responses
    current_lead = User.query.get(session['user_id'])
    if not current_lead.can_access_client_data(response.user_id):
        flash('You are not authorized to re-open this question.')
        return redirect(url_for('dashboard'))
    
    # Check if the question is approved or frozen approved
    if response.review_status not in ['approved', 'approved_after_revision']:
        flash('Only approved or frozen approved questions can be re-opened.')
        return redirect(url_for('dashboard'))
    
    # Re-open the question for review
    response.review_status = 'pending'
    response.is_reviewed = False
    response.is_approved = False
    response.updated_at = datetime.now(timezone.utc)
    
    # If there's an existing chat, reactivate it
    existing_chat = QuestionChat.query.filter_by(response_id=response_id).first()
    if existing_chat:
        ...
        reopen_message = ChatMessage(
            ...
            sender_id=session['user_id'],
            message_type='status_change',
            content='🔓 Question has been re-opened for review by lead',
            is_read_by_client=False,
            is_read_by_lead=True
        )
        db.session.add(reopen_message)
    
    db.session.commit()
    
    # Recalculate scores and update product status
    calculate_and_store_scores(response.product_id, response.user_id)
    update_product_status(response.product_id, response.user_id)
    
    flash('Question has been successfully re-opened for review.')
    return redirect(url_for('review_questionnaire', response_id=response_id))
