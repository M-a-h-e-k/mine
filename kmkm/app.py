...
def calculate_actual_subdimension_scores(product_id, user_id):
    """
    Calculate scores by actual sub-dimensions using CSV structure
...
def get_maturity_level_from_score(score):
    """Map score to maturity level using flooring logic"""
    if score >= 4.5:
        return 5
    elif score >= 4.0:
        return 4
...
def generate_subdimension_roadmap(product_id, user_id):
    """Generate sub-dimension wise roadmap recommendations"""
    ...
    for subdim_name, data in subdimension_scores.items():
        ...
        # Find DSOMM recommendation for next level
        ...
        if current_level < 5:
            ...
            roadmap.append({
                ...
                'recommendation': 'Already optimized - maintain current practices',
                'questions_count': data['question_count']
            })
...
def generate_overall_roadmap(product_id, user_id):
    """Generate 12-month Strategic Security Maturity Plan using new logic"""
    
    # Step 1: Calculate Overall Maturity Level
    overall_score, section_data = calculate_overall_maturity_score(product_id, user_id)
    
    # Current Level = floor(Overall Maturity Score)
    current_level = int(overall_score) if overall_score and overall_score > 0 else 1
    current_level = max(1, min(5, current_level))  # Ensure it's between 1-5
    
    # Get sub-dimension scores for identifying areas below current level
    subdimension_scores = calculate_actual_subdimension_scores(product_id, user_id)
    ...
    # Step 2: Define the Phases based on current level
    phases = [
        {
            'name': 'Phase 1 (0-3 months)', 
            'target_level': current_level, 
            'goal': f'Bring all sub-dimensions up to at least Level {current_level}',
            'purpose': 'Standardize all areas to current maturity level'
        },
        {
            'name': 'Phase 2 (3-6 months)', 
            'target_level': min(5, current_level + 1), 
            'goal': f'Reach Level {min(5, current_level + 1)} maturity',
            'purpose': 'Advance to next DSOMM maturity level'
        },
        {
            'name': 'Phase 3 (6-12 months)', 
            'target_level': 5, 
            'goal': 'Achieve Level 5 - Continuous Improvement',
            'purpose': 'Reach optimized security maturity'
        }
    ]
    ...
    for phase in phases:
        ...
        if phase['name'].startswith('Phase 1'):
            # Step 3: Phase 1 - Identify sub-dimensions below current level
            focus_areas = []
            ...
            for subdim_name, data in subdimension_scores.items():
                ...
                if subdim_level < current_level:
                    ...
                    # Map subdimension to focus area
                    focus_area = get_focus_area_from_subdimension(subdim_name)
                    if focus_area not in focus_areas:
                        focus_areas.append(focus_area)
            
            # Get combined DSOMM recommendations for current level
            combined_recommendations = get_combined_dsomm_recommendations(current_level, focus_areas, dsomm_data)
            ...
            # Step 4 & 5: Phase 2 and Phase 3 - Organization-wide improvement
            all_focus_areas = get_all_focus_areas()
            combined_recommendations = get_combined_dsomm_recommendations(target_level, all_focus_areas, dsomm_data)
...
def dashboard():
    ...
    if role == 'client':
        ...
        # Get ALL products for the assigned client (both complete and incomplete)
        assigned_client = User.query.get(current_lead.assigned_client_id)
        all_client_products = Product.query.filter_by(owner_id=current_lead.assigned_client_id).all()
        
        # Get all responses for the assigned client
...
def product_results(product_id):
    try:
        ...
        try:
            subdimension_scores = calculate_actual_subdimension_scores(product_id, session['user_id'])
        except Exception as e:
...
def product_roadmap(product_id, user_id):
    ...
    try:
        ...
        # Allow access if user is viewing their own roadmap or is admin/lead
        if session_user_id != user_id and session.get('role') not in ['admin', 'lead']:
